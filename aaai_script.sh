...
recurseStyles() {
	...
	if [ ! -e sources/$name.sty ]; then 
	    FILE=`locate /$name.sty | grep '^/usr' | head -n 1`
        if [ ! -z "$FILE" ]; then
...
cp __tmp2 sources/${srcout}.tex


echo "getting figures"
#FIGURES=`grep includegraphics __tmp2 | grep -v ^% | sed 's/.*{\([^}]*\)}.*/\1/'`
FIGURES=`grep -oe 'includegraphics[^{]*{\([^}]*\)}' __tmp2 | sed 's/^[^{]*{\(.*\)}$/\1/'`
for name in $FIGURES; do
    if [ -e "$name" ] ; then
        namei="$name"
    elif [ -e "$name.pdf" ] ; then
        namei="$name.pdf"
    else
        namei="${name}.eps"
    fi
    echo "getting figure $namei"
    namedir=`dirname "$namei"`
#        mkdir -p sources/`echo "$namei" | sed 's/\(.*\)\/.*/\1/'`
    mkdir -p sources/"$namedir"
    cp "$namei" sources/`echo "$namei" | sed 's/\(.*\)\/.*/\1/'`
done
...
#dvips -Ppdf -G0 -tletter ${srcout} -o ${srcout}.ps
#ps2pdf -sPAPERSIZE=letter -dMaxSubsetPct=100 -dCompatibilityLevel=1.2 -dSubsetFonts=false -dEmbedAllFonts=true ${srcout}.ps
cd ..
